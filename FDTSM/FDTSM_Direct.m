function [ y ] = FDTSM_Direct( x, N, region_info )
...
%     %Create the composite frame
    for r = 1:region_info.num_regions
        %Extract frame
        frame_current(:,region_info.lower(r):region_info.upper(r)) = repmat(x(ptr_input(r):ptr_input(r)+N-1,:).*wn, 1, region_info.upper(r)-region_info.lower(r)+1);
    end
    %Dot transpose to rotate by 90.  No dot means complex conjugate
    FRAME_COMP = sum(frame_current.*exp_arr,1).';
