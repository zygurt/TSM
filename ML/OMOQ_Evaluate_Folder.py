...
training_target = 0
feat_start = 5
csv_out_load_file = 'data/Features/MOVs_Eval_All_To_Test.csv'
load_file = 'data/Features/MOVs_Eval_All_To_Test.mat'
network_path = 'models/FCN/2020-05-19_11-15-31TO_TEST_SOURCE/' #MeanOS


fname = 'To_Test_MeanOS_Folder'

models_path = network_path+'Models/'
model_files = [f for f in listdir(models_path) if isfile(join(models_path, f))]
eval_idx=1
for v in range(len(model_files)):
    MODEL_PATH = models_path+model_files[v]
    # Norm_Val_file = "./data/Norm_Vals.p"
    Norm_Val_file = network_path + "Norm_Vals.p"
    ...
    new_folder_name = 'log/EvalFinal/'+fname
    ...
    output_num_as_str = model_files[v][:].split('_')[1].split('.')[0]

    print("Printing: " + new_folder_name + "/Eval_" + output_num_as_str +".csv")
    # sys.exit()
    with open(new_folder_name + "/Eval_" + output_num_as_str +".csv", "a") as results: results.write("Test File, Ref File, Method, TSM, OMOS\n")
    with open(new_folder_name + "/Eval_" + output_num_as_str +".csv", "a") as results:
