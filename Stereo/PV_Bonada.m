function [ y ] = PV_Bonada( x, N, TSM )
%This phase vocoder processes stereo signals using the method proposed by
%J. Bonada, “Audio time-scale modification in the context of professional audio post-
%production,” Ph.D. dissertation, Graduate Division, University of Pompeu Fabra,
...
while(ptr_input<ptr_end)
    ...
    if ptr_input == 1
        ...
        %Bonada stereo calculations
        delta_analysis_phase = frame_phase(:,2)-frame_phase(:,1);
        ...
        %Bonada stereo calculations
        delta_synthesis_phase = Y_phase(:,2)-Y_phase(:,1);
        
        Delta_phase_difference = delta_analysis_phase - delta_synthesis_phase;
        %Split the difference across both channels
        Y_phase(:,1) = Y_phase(:,1)-Delta_phase_difference/2;
        Y_phase(:,2) = Y_phase(:,2)+Delta_phase_difference/2;
