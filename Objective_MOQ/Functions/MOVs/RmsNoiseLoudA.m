function [ MOV ] = RmsNoiseLoudA( Ref, Test, NoiseLoudA, General )
%[ MOV ] = RmsNoiseLoudB( Ref, Test, NoiseLoudB, General )
%   As described by ITU-R BS.1387-1 Section 4.3.1
global debug_var

if debug_var
    disp('    RMSNoiseLoudAsymA');
end
s_test = NoiseLoudA.ThresFac0*Test.Mod+NoiseLoudA.S0;
s_ref = NoiseLoudA.ThresFac0*Ref.Mod+NoiseLoudA.S0;

beta = exp(-NoiseLoudA.alpha.*((Test.E_P-Ref.E_P)./Ref.E_P));
